...
def test_nodectl_info(host):
    ...
    for layout in layouts:
        assert re.search(layout, output_layer_info), \
            "Not identical with imgbase layout"
    ...
    for index in output_index_line_num.split():
        ...
        assert boot_entry in imgbase_layouts, \
            "Bootloader boot entry not correct"
...
def test_nodectl_check(host):
    """
    Purpose:
        RHEVM-16604
        Test the nodectl check subcommand
    """
    # Get the nodectl check status
    cmd_nodectl_check_status = "nodectl check|grep 'Status:'"
    output_nodectl_check_status = host.sendcmd(cmd_nodectl_check_status)
    nodectl_check_status = output_nodectl_check_status.split(':')[-1].strip()
    # Get the mount points status
    cmd_mount_points_status = "nodectl check|grep 'Mount points'"
    output_mount_points_status = host.sendcmd(cmd_mount_points_status)
    mount_points_status = output_mount_points_status.split('...')[-1].strip()
    # Get the basic storage status
    cmd_basic_storage_status = "nodectl check|grep 'Basic storage'"
    output_basic_storage_status = host.sendcmd(cmd_basic_storage_status)
    basic_storage_status = output_basic_storage_status.split('...')[-1].strip()
    # Get the vdsmd status
    cmd_vdsmd_status = "nodectl check|grep 'vdsmd'"
    output_vdsmd_status = host.sendcmd(cmd_vdsmd_status)
    vdsmd_status = output_vdsmd_status.split('...')[-1].strip()
    # Get the vdsm status by systemctl
    cmd_sys_vdsmd_status = "systemctl status vdsmd.service|grep Active"
    output_sys_vdsmd_status = host.sendcmd(cmd_sys_vdsmd_status)
