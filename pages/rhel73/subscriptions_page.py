...
class SubscriptionsPage(PageObject):
    ...
    installed_product_btns = MultiPageElement(tag_name="th")
    ...
    def check_subscription_result(self):
        with self.switch_to_frame(self.frame_right_name):
            installed_product_btn_list = list(self.installed_product_btns)
            for i in installed_product_btn_list[0:]:
                i.click()
            self.wait(period=1)
            rhv_product_name = list(self.spans)[0]
            rhv_register_status = list(self.spans)[4]
            rhel_product_name = list(self.spans)[7]
            rhel_register_status = list(self.spans)[11]

            assert rhv_product_name.text == "Red Hat Enterprise Virtualization", \
                "product name is wrong"
            assert rhv_register_status.text == "Subscribed", \
                "subscription fail"
            assert rhel_product_name.text == "Red Hat Enterprise Linux Server", \
                "product name is wrong"
            assert rhel_register_status.text == "Subscribed", \
                "subscription fail"
